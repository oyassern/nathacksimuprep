...
- Numerical stability when aggregating many small float values (we used compensated summation where needed).
- Balancing realistic clinical difficulty and fast tests for development.
- Designing instructor analytics that are informative without being overwhelming.
...
- Keeping computations numerically stable matters in production analytics (use stable accumulators for long-running aggregates).
...
- Mobile/offline support and tighter LMS (LTI) integrations for larger adoption.
